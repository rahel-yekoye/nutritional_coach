...
HEALTHCHECK --interval=30s --timeout=3s --start-period=5s --retries=3 \
  CMD node -e "require('http').get('https://nutritional-coach.onrender.com/health', (r) => {process.exit(r.statusCode === 200 ? 0 : 1)})"
...
services:
  api:
    ...
    healthcheck:
      test: ["CMD", "wget", "--quiet", "--tries=1", "--spider", "https://nutritional-coach.onrender.com/health"]
      interval: 30s
      timeout: 10s
      retries: 3
```
